...
class IncomeProtection(Base):
    ...
    date_started = Column(Date(), index=True)
    created_date = Column(DateTime, default=func.now())
    updated_date = Column(DateTime, default=func.now(), onupdate=func.now())
    
    user = relationship("User", back_populates=__tablename__, cascade="all", lazy="select")
    income_protection_provision = relationship("IncomeProtectionProvision", back_populates=__tablename__, cascade="all", lazy="select")